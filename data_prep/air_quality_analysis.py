...
def setup_plotting() -> None:
    ...
    print("[OK] Plotting configuration complete")
...
def load_station_data(
    ...
    for station in stations:
        try:
            ...
            all_data.append(df)
            print(f"[OK] Loaded {station}: {len(df)} records")
        except FileNotFoundError:
            print(f"[ERROR] File not found: {data_dir}{station}.csv")
        except Exception as e:
            print(f"[ERROR] Error loading {station}: {e}")
    
    # Combine all data
    if all_data:
        combined_df = pd.concat(all_data, ignore_index=True)
        print(f"\n[OK] Total combined records: {len(combined_df)}")
        print(f"[OK] Columns: {combined_df.columns.tolist()}")
    else:
...
def plot_time_series(station_data: Dict[str, pd.DataFrame]) -> None:
    ...
    if not station_data:
        print("[ERROR] No station data available for plotting")
        return
    ...
    for var_name, var_columns in variable_mapping.items():
        ...
        for i, station in enumerate(stations):
            ...
            if actual_column is not None and not df[actual_column].dropna().empty:
                ...
                print(f"[WARNING]: {station} does not have {var_name} data or data is empty")
        ...
        if station_count > 0:
            ...
            plt.tight_layout()
            plt.show()
            print(f"[OK] Generated time series subplots for {var_name} ({station_count} stations)")
        else:
            plt.close(fig)
            print(f"[WARNING] No data available for {var_name} - skipping plot")
    ...
    for station in stations:
        ...
        if len(available_vars) > 0:
            ...
            print(f"[OK] Generated overview plot for {station}")
        else:
            print(f"[WARNING]: {station} has no recognizable variables with valid data")
...
def plot_distributions(station_data: Dict[str, pd.DataFrame]) -> None:
    ...
    for var in variables:
        ...
        print(f"[OK] Generated distribution plots for {var}")
...
def analyze_correlations(station_data: Dict[str, pd.DataFrame]) -> None:
    ...
    # Check if we have any data
    if not station_data:
        print("[ERROR] No station data available for correlation analysis")
        return
    ...
    for station in stations:
        ...
        if len(available_vars) > 1:  # Need at least 2 variables for correlation
            ...
            if correlation_data.isna().all().all():
                print(f"[WARNING]: {station} has no valid correlations")
                continue
    ...
    print(f"[OK] Generated separated correlation matrices for all stations and variables")
...
def analyze_missing_data(station_data: Dict[str, pd.DataFrame]) -> None:
    ...
    plt.tight_layout()
    plt.show()
    print("[OK] Generated missing data visualizations")
...
def generate_pdf_report(
    ...
    print(f"[OK] PDF report saved as: {filename}")
    print(f"[OK] Report contains comprehensive analysis with visualizations")
    print(f"{'='*70}")
...
def generate_executive_summary_pdf(
    ...
    """
    print(f"\n{'='*70}")
    print("GENERATING EXECUTIVE SUMMARY PDF (2 PAGES MAX)")
    print(f"{'='*70}")
...
• Regularly maintain and calibrate sensors"""
    ...
    print(f"[OK] Executive summary PDF saved as: {filename}")
    print(f"{'='*70}")
...
def main() -> None:
    ...
    try:
        ...
        if not station_data or combined_df.empty:
            print("[ERROR] No data loaded. Please check data files and paths.")
            return
        ...
        generate_pdf_report(station_data, combined_df, filename='data_report.pdf')
